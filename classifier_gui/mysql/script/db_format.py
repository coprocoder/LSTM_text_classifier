...
class DB_formalizer:
    ...
    @staticmethod
    def normalise_body_on_db(classifier_object, begin, end):
        ...
        for i in range(begin, end):
            ...
            stmt_update = "UPDATE {database}.{table} SET {article_a_body} = {text} WHERE id = {id};".format(
                ...
                article_a_body=classifier_object.dbinfo['body_field'],
                id=i)
            cur.execute(stmt_update)

            # добавить SET SQL_SAFE_UPDATES = 0;  если не изменяет записи

            stmt_cut = "UPDATE {database}.{table} SET {ticket_queue_name} = substr({ticket_queue_name},1,instr({ticket_queue_name},'::') - 1) " \
                          "WHERE ticket_queue_name LIKE '%::%' AND id = {id};".format(
                database=classifier_object.dbinfo['database'],
                table=classifier_object.dbinfo['table'],
                id=i,
                article_a_body=classifier_object.dbinfo['body_field'],
                ticket_queue_name=classifier_object.dbinfo['queue_field'])
